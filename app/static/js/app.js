...
window.Blog = angular.module('angular_blog', ['ngResource', 'ngRoute', 'ui.bootstrap', 'ui.date', 'restangular', 'LocalStorageModule'])
    run(function ($location, Restangular, AuthService) {
        Restangular.setFullRequestInterceptor(function (element, operation, route, url, headers, params, httpConfig) {
            headers['Authorization'] = 'Basic' + AuthService.getterToken();
            return {
                headers: headers
            };
        });
        ...
        Restangular.setErrorInterceptor(function (response, deferred, responseHandler) {
            if(response.config.bypassInterseptor) {
                return true;
            } else {
